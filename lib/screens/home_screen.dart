          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => PeopleListScreen(userId: widget.userId),
                ),
              );
            },
              ...
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) => SettingsScreen(
                    ...
                    showMessage: (text, type) => showMessageBox(
                      context,
                      text,
                      type,
                    ), // Pass showMessageBox
          ...
          : Container(
              ...
              child: StreamBuilder<List<Person>>(
                ...
                builder: (context, snapshot) {
                  ...
                  final allReminders = DateHelpers.calculateReminders(
                    people,
                  ); // Get all reminders

                  // Filter to get only the actual event dates (reminders with offset 'On Day')
                  final allEvents = allReminders
                      .where((r) => r.offset == 'On Day')
                      .toList();

                  // Group events by person and find the next event for each
                  final Map<String, Reminder> nextEventPerPerson = {};
                  final DateTime normalizedToday = DateHelpers.normalizeDate(
                    DateTime.now(),
                  );

                  for (var event in allEvents) {
                    final DateTime eventDateOnly = DateHelpers.normalizeDate(
                      event.originalDate,
                    );
                    final Reminder? existingEvent =
                        nextEventPerPerson[event.personName];

                    if (existingEvent == null) {
                      // If no event for this person yet, add it
                      nextEventPerPerson[event.personName] = event;
                    } else {
                      final DateTime existingEventDateOnly =
                          DateHelpers.normalizeDate(existingEvent.originalDate);
                      ...
                      if (eventDateOnly.isAfter(normalizedToday) &&
                          existingEventDateOnly.isBefore(normalizedToday)) {
                        nextEventPerPerson[event.personName] = event;
                      } else if (eventDateOnly.isAfter(normalizedToday) &&
                          existingEventDateOnly.isAfter(normalizedToday)) {
                        if (eventDateOnly.isBefore(existingEventDateOnly)) {
                          nextEventPerPerson[event.personName] = event;
                        }
                      } else if (eventDateOnly.isAtSameMomentAs(
                            normalizedToday,
                          ) &&
                          existingEventDateOnly.isBefore(normalizedToday)) {
                        nextEventPerPerson[event.personName] = event;
                  ...
                  upcomingEvents.sort(
                    (a, b) => a.originalDate.compareTo(b.originalDate),
                  );
                                        ...
                                        child: Padding(
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              ...
                                              Text(
                                                'Date: ${DateFormat.yMd().format(event.originalDate)}',
                                                style: const TextStyle(
                                                  fontSize: 14,
                                                  color: Colors.grey,
                                                ),
